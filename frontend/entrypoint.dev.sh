...
rm -rf ./public/env-config-dev.js
touch ./public/env-config-dev.js

# Add assignment 
echo "window._env_ = {" >> ./public/env-config-dev.js
...
while IFS='=' read -r varname varvalue || [ -n "$varname" ]; do
  ...
  if [ -n "$varvalue" ]; then
    ...
    echo "  $varname: \"$value\"," >> ./public/env-config-dev.js
  fi
done < .env.dev

echo "}" >> ./public/env-config-dev.js
